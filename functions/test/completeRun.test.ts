...
describe("completeRun callable boundary", () => {
  ...
  it("rejects a suspended caller with permission-denied before writing any completion artifacts (defence-in-depth)", async () => {
    await firestore.doc(`users/${USER_UID}`).set({ accountStatus: "suspended" });

    await expectRejectsCode(
      () => callCompleteRun({ auth: { uid: USER_UID }, data: validPayload() }),
      "permission-denied",
    );

    const activities = await firestore.collection("activities").where("ownerUid", "==", USER_UID).get();
    assert.equal(activities.empty, true);
  });

  it("still completes normally for an explicitly unsuspended caller", async () => {
    await firestore.doc(`users/${USER_UID}`).set({ accountStatus: "active" });
